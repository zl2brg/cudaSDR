...
static pthread_t wideband_thread_id = 0;
    ...
    while (1) {
        ...
        if (rc != run) {
            ...
            if (run) {
                ...
                if (pthread_create(&wideband_thread_id, NULL, wideband_thread, NULL) < 0) {
                    dbg_printf(1, "***** ERROR: Create Wideband thread\n");
                }
            } else {
                ...
                pthread_join(wideband_thread_id, NULL);
...
// Send Protocol 2 wideband ADC data to the PC.
// Packet format: 16-byte header + 512 x int16_t samples = 1040 bytes total.
//   Bytes  0-3 : sequence number (big-endian)
//   Bytes  4-15: zeros (timestamp / reserved)
//   Bytes 16-1039: 512 signed 16-bit noise samples (little-endian)
// 32 packets assemble to BIGWIDEBANDSIZE (16384 samples) in cudaSDR's DataIO.
// Sent at ~5 fps (200ms between bursts of 32).
void* wideband_thread(void *data) {
    int sock;
    int yes = 1;
    unsigned char buffer[1040];
    unsigned long seqnum = 0;
    unsigned int seed = ((uintptr_t)&seed) & 0xFFFFFF;
    int pkt, i;
    int16_t sample;

    dbg_printf(1, "-- Start wideband_thread\n");

    sock = socket(AF_INET, SOCK_DGRAM, 0);
    if (sock < 0) {
        dbg_printf(1, "***** ERROR: wideband_thread: socket\n");
        return NULL;
    }
    setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (void*)&yes, sizeof(yes));
    setsockopt(sock, SOL_SOCKET, SO_REUSEPORT, (void*)&yes, sizeof(yes));
    ...
    while (run) {
        ...
        for (pkt = 0; pkt < 32 && run; pkt++) {
            // 16-byte header: sequence number then zeros
            memset(buffer, 0, 16);
            ...
            // 512 signed 16-bit noise samples, each generated as a single
            // rand_r() call to avoid spectral artefacts from byte correlation.
            for (i = 0; i < 512; i++) {
                seed = (unsigned int)(seed * 1103515245U + 12345U);
                // Scale to ~-40 dBFS so the noise floor is visible
                sample = (int16_t)((int)(seed >> 16) - 32768) >> 6;
                buffer[16 + i*2 + 0] = (unsigned char)(sample & 0xFF);
                buffer[16 + i*2 + 1] = (unsigned char)((sample >> 8) & 0xFF);
            }

            if (sendto(sock, buffer, 1040, 0,
                       (struct sockaddr*)&addr_new, sizeof(addr_new)) < 0) {
                dbg_printf(1, "***** ERROR: wideband_thread sendto\n");
                break;
            }
        }
    ...
    close(sock);
    dbg_printf(1, "-- wideband_thread exit\n");
    return NULL;
}
