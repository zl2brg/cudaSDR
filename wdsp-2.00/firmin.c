...
void plan_fircore(FIRCORE a) {
  ...
  a->crev = fftw_plan_dft_1d(2 * a->size, (fftw_complex *)a->accum, (fftw_complex *)a->out, FFTW_BACKWARD, FFTW_PATIENT);
  a->masks_ready = 0;
  // Min-phase tooling is only needed when mp!=0. Eager create_minphase(nc,4)
  // builds four FFTW_PATIENT plans of size nc*4 on every bandpass/EQ/CFIR
  // (even with mp=0) and dominated OpenChannel time without wisdom.
