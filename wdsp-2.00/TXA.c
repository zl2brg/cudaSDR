...
void create_txa(int channel) {
                                ...
                                0.200);                   // muted gain
  {
    ...
    // Keep 1.29-sized EQ defaults at create time. WDSP 2.00's 16384-tap
    // min-phase EQ forces create_minphase(16384,4) → four FFTW_PATIENT plans
    // of length 65536 inside OpenChannel and freezes the UI for a minute+.
