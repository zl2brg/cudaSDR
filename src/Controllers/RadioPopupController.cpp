...
void RadioPopupController::bind(RadioPopupWidget* view, SliceModel* sliceModel, Settings* model)
{
    ...
    if (m_sliceModel) {
        ...
        connect(m_view, &RadioPopupWidget::rttyDecodeRequested, this, [this](bool enabled) {
            const int rx = m_view ? m_view->getReceiver() : (m_sliceModel ? m_sliceModel->id() : 0);
            if (m_sliceModel) {
                m_sliceModel->setRttyDecodeEnabled(enabled);
            }
            if (m_model) {
                m_model->setRttyDecode(rx, enabled);
            }
            if (enabled) {
                if (m_model) {
    ...
    if (dispView && m_sliceModel) {
        ...
        if (m_model) {
            ...
            dispView->setPanSMeterSize(m_model->getPanadapterSMeterSize());
        ...
        connect(dispView, &DisplayOptionsWidget::panSMeterSizeRequested, this, [this](int val) {
            if (m_model) m_model->setPanadapterSMeterSize(val);
        });
