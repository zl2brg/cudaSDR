...
void DisplaySettingsController::bind(DisplayTabWidget* container, RadioModel* radioModel, Settings* model)
{
    ...
    if (m_displayView) {
        ...
        m_displayView->setPanSMeterSize(m_model->getPanadapterSMeterSize());
        ...
        connect(m_model, &Settings::panadapterSMeterSizeChanged, m_displayView, &DisplayOptionsWidget::setPanSMeterSize);
        ...
        connect(m_displayView, &DisplayOptionsWidget::panSMeterSizeRequested, this, [this](int val) {
            m_model->setPanadapterSMeterSize(val);
        });
