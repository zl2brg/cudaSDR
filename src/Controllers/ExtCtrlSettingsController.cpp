...
void ExtCtrlSettingsController::bind(ExtCtrlWidget* view, RadioModel* radioModel, Settings* model)
{
    m_view = view;
    ...
    m_view->setPennyOCEnabled(m_radioModel ? m_radioModel->txParams().pennyOCenabled : m_model->getPennyOCEnabled());
    m_view->setRxPins(m_radioModel ? m_radioModel->txParams().rxJ6pinList : m_model->getRxJ6Pins());
    m_view->setTxPins(m_radioModel ? m_radioModel->txParams().txJ6pinList : m_model->getTxJ6Pins());

    // --- 2. View -> Model (User interaction events) ---
    connect(m_view, &ExtCtrlWidget::pennyOCEnabledRequested, this, [this](bool enabled) {
        if (m_radioModel) {
            m_radioModel->txParams().pennyOCenabled = enabled;
        }
    ...
    connect(m_view, &ExtCtrlWidget::rxPinsRequested, this, [this](const QList<int>& pins) {
        if (m_radioModel) {
            m_radioModel->txParams().rxJ6pinList = pins;
        }
    ...
    connect(m_view, &ExtCtrlWidget::txPinsRequested, this, [this](const QList<int>& pins) {
        if (m_radioModel) {
            m_radioModel->txParams().txJ6pinList = pins;
        }
