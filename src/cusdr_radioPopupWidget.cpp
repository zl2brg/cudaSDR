...
void RadioPopupWidget::createOptionsBtnGroup() {
    ...
    m_cwLogCheckBox->setStyleSheet("color: rgba(220, 220, 220, 255);");
    m_cwLogCheckBox->setToolTip(tr("Log decoded CW text and callsigns to ~/.cudaSDR/logs/"));
    m_cwLogCheckBox->setChecked(m_sliceModel ? m_sliceModel->cwLogToFile() : false);
    connect(m_cwLogCheckBox, &QCheckBox::clicked, this, [this](bool checked) {
...
void RadioPopupWidget::setCwLog(bool enable) {
    if (m_cwLogCheckBox) {
        const QSignalBlocker blocker(m_cwLogCheckBox);
        m_cwLogCheckBox->setChecked(enable);
    }
}
